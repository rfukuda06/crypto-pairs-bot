...
- **Top panel — the signal.** The z-score of the LTC/XLM spread: how far the two coins have drifted from their usual relationship (0 = normal). When it stretches past the ±2 bands, the bot bets they'll converge and opens a trade — **▲** short, **▼** long, **○** close.
...
That p-value is the **minimum across all 55 candidate pairs**, so it is itself a data-snooped statistic: after a Šidák multiple-testing correction it is ≈ 0.70 — *not* significant. That is not a caveat to bury; it is the same lesson as the parameter grid. The pair looked cointegrated in-sample largely by luck, which is exactly why it loses out-of-sample. The `screen` command prints this correction next to the raw p-value so the number can't be read as more than it is.
...
Every claim above rests on structural properties that are *enforced*, not trusted. 75 tests, all passing.
...
| **In / out-of-sample split** | Pair + β chosen on the training window only; exact non-overlapping partition with in-sample preceding out-of-sample. | `tests/test_split.py`, `tests/test_cli_selection.py` |
...
| **Network resilience** | Retries a flaky feed then succeeds; gives up with a clean error after 5 failed attempts; raises on fewer than 2 bars. | `tests/test_live_feed.py` |
